###-----------------------------------------------------------
### The following code is based on the following repo: https://github.com/tyler-hayes/Embedded-CL
### which itself is the corresponds to the paper 
### Hayes, Tyler L., and Christopher Kanan. "Online continual learning for embedded devices." arXiv preprint arXiv:2203.10681 (2022).
### The original code is adapted for our purposes.
...
def make_data_list(keys, data_list, balanced_shots=False):
    new_data_list = []
    for k in keys:
        data = data_list[k]
        data_ids = data[:-1]
        frames = data[-1][:60] if balanced_shots else data[-1]
        for frame in frames:
...
def instance_few_shot_ordering(data_list, seed, num_classes, k_shot=1, balanced_shots=False):
    # randomly grab one video per class
    new_data_list = []
    all_paths = []
    cls_paths = find_videos_for_class(data_list, num_classes)
    ...
    for i, class_id in enumerate(cls_order):
        ...
        collected_shots = 0
        curr_video_data = []
        while collected_shots < k_shot and curr_ix<len(curr_paths)-1:
            while len(curr_video_data) == 0:  # some instances do not contain frames, so ensure we grab one with frames
                # arbitrarily grab first k instances since the instances were shuffled
                curr_ix += 1
                curr_video_data = make_data_list([curr_paths[curr_ix]], data_list)
            

            all_paths.append(curr_paths[curr_ix])
            collected_shots += 1
            curr_video_data = []

    random.shuffle(all_paths) # shuffle collected videos, so that videos are not ordered by classes
    new_data_list = make_data_list(all_paths, data_list, balanced_shots)
    # assert k_shot*num_classes == check_number_of_instances(new_data_list)
    return new_data_list
...
def make_dataset(data_list, data_type, num_classes, k_shot, ordering='instance', seed=666, balanced_shots=False):
    """
    data_list
    ...
    if ordering not in ['iid', 'class_iid', 'instance', 'class_instance', 'instance_small', 'instance_small_121', 'instance_few_shot']:
    ...
    elif ordering == 'instance_few_shot':
        return instance_few_shot_ordering(data_list, seed, num_classes, k_shot, balanced_shots)
...
class OpenLorisDataset(data.Dataset):
    ...
    def __init__(self, root, train=True, ordering=None, transform=None, target_transform=None, num_classes=40, seed=10,
                 label_level='class', k_shot=None, balanced_shots=False):
        if train:
            data_type = 'train'
        ...
        frame_list = make_dataset(data_list, data_type, num_classes, ordering=ordering, seed=seed, k_shot=k_shot, balanced_shots=balanced_shots)
