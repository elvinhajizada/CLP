...
from openloris.FSIOLDataset import FSIOL
...
def get_stream_data_loader(images_dir, training, ordering=None, batch_size=128, shuffle=False, augment=False,
                           ...
                           dataset='openloris', k_shot=None, balanced_shots=False):
    ...
    if dataset == 'openloris':
        dataset = OpenLorisDataset(images_dir, train=training, ordering=ordering, transform=transform, seed=seed,
                                   label_level=label_level, k_shot=k_shot, balanced_shots=balanced_shots)
    elif dataset == 'fsiol':
...
def f1_score(ood_scores_in, ood_scores_out, th):
    ...
    y_scores = np.concatenate((ood_scores_in>th.numpy(), ood_scores_out.numpy()))
...
def update_and_save_accuracies_ood(ood_scores_in_intra, true_labels_in_intra, ood_scores_out_intra,
...
def update_and_save_accuracies_semi_supervised(probas, y_test_init, save_dir, index, accuracies,
                                               acc_index):
    top1, top5 = accuracy(probas, y_test_init, topk=(1, 5))
    print('\nIndex: %d -- top1=%0.2f%% -- top5=%0.2f%%' % (index, top1, top5))
    accuracies[list(accuracies)[acc_index]].append(float(top1))
...
def save_model_size(save_dir, index, accuracies, final_model_size, base_model_size=None):
    
    if base_model_size is None:
        accuracies['final_model_size'].append(int(final_model_size))
        print('\nFinal model size= %d' % (final_model_size))
        
    else:
        accuracies['base_model_size'].append(int(base_model_size))
        print('\nBase model size= %d -- Final model size= %d' % (base_model_size, final_model_size))
